...
class Controller:
    ...
    def run_inference(self):
        ...
        while True:
            try:
                ...
                img_final = self.display_prediction(img, prediction_postprocessed)
                # cv2.imshow("inamg3", img_final)
                # Apply callbacks
    ...
    def display_prediction(self, img: np.ndarray, prediction_postprocessed: dict):
        ...
        # self.results_display.show(img, prediction_postprocessed)
        # cv2.imshow("second img", img_augmented)
        # Recording
        if self.path_out:
