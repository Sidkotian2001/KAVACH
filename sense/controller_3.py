...
class Controller:
    ...
    def run_inference(self):
        ...
        current_time = 0
        init_time = time.time()
        prev_max_class = 'Non-Violence Videos'
        ...
        while True:
            try:
                ...
                #Find the classs with max probablity
                pred = prediction_postprocessed['sorted_predictions']
                max_class, max_prob = max(pred, key=lambda x: x[1])

                #Only if probability of a class is greater than 0.9 for a period of 2 seconds
                if max_prob > 0.9:

                    if max_class == prev_max_class:

                        #check if the start frame for video recording has started
                        # if frame_recording_start == False:
                        #     self.frame_start = current_frame_number
                        #     frame_recording_start = True
                        ...
                        #Start the current recording frame
                        self.frame_start = current_frame_number

                        # if frame_recording_end:
                            # self.frame_end = current_frame_number
                            # frame_recording_end = False
